...
JOIN job USING (job_id);
-- Nested Loop  (cost=2396.83..2617.49 rows=103 width=91) (actual time=0.098..0.098 rows=0 loops=1)
--   ->  Hash Join  (cost=2396.54..2555.48 rows=103 width=25) (actual time=0.097..0.097 rows=0 loops=1)
--         Hash Cond: (vacancy.vacancy_id = vacancy_1.vacancy_id)
--         ->  Seq Scan on vacancy  (cost=0.00..135.86 rows=8786 width=9) (actual time=0.021..0.022 rows=1 loops=1)
--         ->  Hash  (cost=2395.26..2395.26 rows=103 width=20) (actual time=0.066..0.066 rows=0 loops=1)
--               Buckets: 1024  Batches: 1  Memory Usage: 8kB
--               ->  GroupAggregate  (cost=2391.65..2394.23 rows=103 width=20) (actual time=0.065..0.065 rows=0 loops=1)
--                     Group Key: vacancy_1.vacancy_id
--                     ->  Sort  (cost=2391.65..2391.91 rows=103 width=12) (actual time=0.063..0.063 rows=0 loops=1)
--                           Sort Key: vacancy_1.vacancy_id
--                           Sort Method: quicksort  Memory: 25kB
--                           ->  Nested Loop Left Join  (cost=23.60..2388.21 rows=103 width=12) (actual time=0.056..0.056 rows=0 loops=1)
--                                 Join Filter: (read_message.account_id = account.account_id)
--                                 ->  Nested Loop  (cost=23.31..2353.13 rows=103 width=12) (actual time=0.055..0.055 rows=0 loops=1)
--                                       ->  Index Only Scan using account_pkey on account  (cost=0.29..8.30 rows=1 width=4) (actual time=0.027..0.029 rows=1 loops=1)
--                                             Index Cond: (account_id = 8)
--                                             Heap Fetches: 1
--                                       ->  Hash Right Join  (cost=23.03..2343.80 rows=103 width=12) (actual time=0.023..0.023 rows=0 loops=1)
--                                             Hash Cond: (message.vacancy_id = vacancy_1.vacancy_id)
--                                             Filter: ((message.account_id <> 8) OR (message.account_id IS NULL))
--                                             ->  Seq Scan on message  (cost=0.00..1944.08 rows=100108 width=12) (never executed)
--                                             ->  Hash  (cost=22.91..22.91 rows=9 width=8) (actual time=0.014..0.014 rows=0 loops=1)
--                                                   Buckets: 1024  Batches: 1  Memory Usage: 8kB
--                                                   ->  Nested Loop  (cost=1.13..22.91 rows=9 width=8) (actual time=0.013..0.013 rows=0 loops=1)
--                                                         ->  Nested Loop  (cost=0.84..17.50 rows=9 width=12) (actual time=0.013..0.013 rows=0 loops=1)
...
--                                                                           Index Cond: (company_id = hr_manager.company_id)
--                                                                           Heap Fetches: 0
--                                                               ->  Index Scan using vacancy_company_id_index on vacancy vacancy_1  (cost=0.29..0.80 rows=9 width=12) (never executed)
--                                                                     Index Cond: (company_id = company.company_id)
--                                                         ->  Index Only Scan using job_pkey on job job_1  (cost=0.29..0.60 rows=1 width=4) (never executed)
